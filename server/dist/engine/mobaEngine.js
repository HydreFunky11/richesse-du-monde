...
exports.MobaEngine = void 0;
const mobaConstants_1 = require("./mobaConstants");
...
class MobaEngine {
    ...
    addPlayer(id, username, championId = "ignis") {
        ...
        this.state.log.push(`${username} a rejoint l'équipe ${team.toUpperCase()} avec ${mobaConstants_1.CHAMPIONS[championId].name}.`);
        return p;
    }
    addBot(team, championId) {
        const blueCount = this.state.players.filter(p => p.team === "blue").length;
        const redCount = this.state.players.filter(p => p.team === "red").length;
        const assignedTeam = team || (blueCount <= redCount ? "blue" : "red");
        const allChamps = Object.keys(mobaConstants_1.CHAMPIONS);
        const chosenChamp = championId || allChamps[Math.floor(Math.random() * allChamps.length)];
        const botId = `bot_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
        const botName = `Bot ${mobaConstants_1.CHAMPIONS[chosenChamp].name}`;
    ...
    selectChampion(playerId, champId) {
        const p = this.state.players.find(pl => pl.id === playerId);
        if (!p || this.state.status !== "LOBBY")
            return;
        if (!mobaConstants_1.CHAMPIONS[champId])
            return;
        p.championId = champId;
        this.applyChampionStats(p, champId);
        this.state.log.push(`${p.username} a sélectionné ${mobaConstants_1.CHAMPIONS[champId].name}.`);
    }
    getSpawnPoint(team) {
        return team === "blue" ? { x: 180, y: LANE_Y } : { x: 2220, y: LANE_Y };
    }
    createPlayerEntity(id, username, team, championId, isBot) {
        const spawn = this.getSpawnPoint(team);
        const champ = mobaConstants_1.CHAMPIONS[championId] || mobaConstants_1.CHAMPIONS.ignis;
        const p = {
            id,
            username,
            team,
    ...
    applyChampionStats(p, champId) {
        const champ = mobaConstants_1.CHAMPIONS[champId];
        if (!champ)
            return;
        ...
        for (const itemId of p.items) {
            const it = mobaConstants_1.MOBA_ITEMS[itemId];
            if (!it || !it.stats)
                continue;
            if (it.stats.hp)
                p.maxHp += it.stats.hp;
            if (it.stats.mana)
                p.maxMana += it.stats.mana;
            if (it.stats.attackDamage)
                p.attackDamage += it.stats.attackDamage;
            if (it.stats.abilityPower)
                p.abilityPower += it.stats.abilityPower;
            if (it.stats.armor)
                p.armor += it.stats.armor;
            if (it.stats.magicResist)
                p.magicResist += it.stats.magicResist;
            if (it.stats.attackSpeed)
                p.attackSpeed += it.stats.attackSpeed;
            if (it.stats.moveSpeed)
                p.moveSpeed += it.stats.moveSpeed;
        }
    }
    startGame() {
    ...
    // --- Input Handlers ---
    handleMove(playerId, targetX, targetY) {
        const p = this.state.players.find(pl => pl.id === playerId);
        if (!p || !p.isAlive || p.isStunned)
            return;
    ...
    handleCastSpell(playerId, spellKey, mouseX, mouseY, targetId) {
        const p = this.state.players.find(pl => pl.id === playerId);
        if (!p || !p.isAlive || p.isStunned || p.isSilenced)
            return;
        if (p.spellsLevel[spellKey] <= 0)
            return;
        if (p.spellsCooldown[spellKey] > 0)
            return;
        const champ = mobaConstants_1.CHAMPIONS[p.championId];
        if (!champ)
            return;
    ...
    handleSummonerSpell(playerId, key, mouseX, mouseY) {
        const p = this.state.players.find(pl => pl.id === playerId);
        if (!p || !p.isAlive || p.isStunned)
            return;
        const sSpell = p.summonerSpells[key];
        if (!sSpell || sSpell.cooldown > 0)
            return;
        p.isRecalling = false;
        if (sSpell.id === "flash") {
    ...
    handleRecall(playerId) {
        const p = this.state.players.find(pl => pl.id === playerId);
        if (!p || !p.isAlive || p.isStunned || p.isRecalling)
            return;
        p.isRecalling = true;
        p.recallProgress = 0;
        p.targetX = null;
    ...
    handleUpgradeSpell(playerId, spellKey) {
        const p = this.state.players.find(pl => pl.id === playerId);
        if (!p || p.availableSpellPoints <= 0)
            return;
        if (spellKey === "r" && p.level < 6)
            return;
        if (p.spellsLevel[spellKey] >= 5)
            return;
        if (spellKey === "r" && p.spellsLevel[spellKey] >= 3)
            return;
        p.spellsLevel[spellKey]++;
        p.availableSpellPoints--;
        this.addFloatingText(p.x, p.y - 30, `${spellKey.toUpperCase()} Amélioré !`, "#F59E0B");
    }
    handleBuyItem(playerId, itemId) {
        const p = this.state.players.find(pl => pl.id === playerId);
        if (!p || !p.isAlive)
            return;
    ...
    updatePlayers() {
        for (const p of this.state.players) {
            ...
            if (p.isStunned) {
                p.stunRemaining--;
                if (p.stunRemaining <= 0)
                    p.isStunned = false;
            }
            if (p.isRooted) {
                p.rootRemaining--;
                if (p.rootRemaining <= 0)
                    p.isRooted = false;
            }
            if (p.isInvulnerable) {
            ...
            for (const k of ["q", "w", "e", "r"]) {
                if (p.spellsCooldown[k] > 0)
                    p.spellsCooldown[k]--;
            }
            if (p.summonerSpells.d.cooldown > 0)
                p.summonerSpells.d.cooldown--;
            if (p.summonerSpells.f.cooldown > 0)
                p.summonerSpells.f.cooldown--;
            if (p.attackCooldown > 0)
                p.attackCooldown--;
            // Recall Channeling
            if (p.isRecalling) {
    ...
    updateBotAi(bot) {
        ...
        if (bot.hp < bot.maxHp * 0.3) {
            ...
            if (Math.hypot(bot.x - base.x, bot.y - base.y) < 250) {
                const itemKeys = Object.keys(mobaConstants_1.MOBA_ITEMS);
                for (const ik of itemKeys) {
                    if (bot.gold >= mobaConstants_1.MOBA_ITEMS[ik].cost && bot.items.length < 6) {
    ...
    findMinionTarget(m) {
        ...
        const enemyTurret = this.state.turrets.find(t => t.team !== m.team && t.hp > 0 && Math.hypot(t.x - m.x, t.y - m.y) < 400);
        if (enemyTurret)
            return enemyTurret;
        // 3. Enemy Nexus in range 400
        const enemyNexus = this.state.nexuses.find(n => n.team !== m.team && n.hp > 0 && Math.hypot(n.x - m.x, n.y - m.y) < 400);
        if (enemyNexus)
            return enemyNexus;
        ...
        for (const ep of enemyChamps) {
            if (Math.hypot(ep.x - m.x, ep.y - m.y) < 250)
                return ep;
        }
        return null;
    }
    minionAttack(m, target) {
        if (m.type === "caster") {
            this.createHomingProjectile(m.id, m.team, target.id || "target", m.x, m.y, target.x, target.y, 8, m.damage, "magic", "#818CF8", "bullet");
        }
        else {
    ...
    updateTurrets() {
        for (const t of this.state.turrets) {
            ...
            const minionsInRange = this.state.minions.filter(m => m.team !== t.team && Math.hypot(m.x - t.x, m.y - t.y) <= t.range);
            if (minionsInRange.length > 0) {
                target = minionsInRange[0];
            }
            else {
                // Target enemy champions in range
                const champsInRange = this.state.players.filter(p => p.team !== t.team && p.isAlive && Math.hypot(p.x - t.x, p.y - t.y) <= t.range);
            ...
            if (target && t.attackCooldown <= 0) {
                ...
                this.createHomingProjectile(t.id, t.team, target.id, t.x, t.y, target.x, target.y, 9, t.damage, "physical", t.team === "blue" ? "#38BDF8" : "#F43F5E", "turret_beam");
            }
        }
    }
    updateJungleMonsters() {
        for (const monster of this.state.jungleMonsters) {
            ...
            const nearbyPlayer = this.state.players.find(p => p.isAlive && Math.hypot(p.x - monster.x, p.y - monster.y) < monster.range + 80);
            if (nearbyPlayer) {
    ...
    handleAttackPassive(attacker, target) {
    ...
    executeSpellEffect(player, champ, key, spell, mouseX, mouseY, targetId) {
    ...
    dealDamage(rawDmg, type, target, source) {
        if (!target || target.hp <= 0)
            return;
    ...
    handleEntityDeath(victim, killer) {
        ...
        if (victimPlayer) {
            ...
            if (killerPlayer) {
                ...
                if (killerPlayer.team === "blue")
                    this.state.killsBlue++;
                else
                    this.state.killsRed++;
