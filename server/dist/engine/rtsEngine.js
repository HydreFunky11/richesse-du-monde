...
exports.RtsEngine = exports.BUILDING_CONFIGS = exports.UNIT_CONFIGS = exports.TECH_TREE = void 0;
exports.TECH_TREE = {
    advanced_mining: {
        id: 'advanced_mining',
...
class RtsEngine {
    ...
    addPlayer(id, username, color) {
        ...
        const factions = ['nanite', 'aegis', 'phantom', 'vanguard'];
    ...
    selectFaction(playerId, faction) {
        const player = this.state.players.find(p => p.id === playerId);
        if (!player || this.state.status !== 'LOBBY')
            return false;
    ...
    createBuilding(playerId, type, x, y, instant = false) {
        ...
        if (!player)
            return null;
        const conf = exports.BUILDING_CONFIGS[type];
        if (!conf)
            return null;
    ...
    createUnit(playerId, type, x, y) {
        ...
        if (!player)
            return null;
        const conf = exports.UNIT_CONFIGS[type];
        if (!conf)
            return null;
        let hp = conf.hp;
        let shield = conf.shield;
    ...
    handleOrder(playerId, unitIds, orderType, targetX, targetY, targetId) {
        const player = this.state.players.find(p => p.id === playerId);
        if (!player)
            return false;
    ...
    handleBuild(playerId, buildingType, x, y) {
        const player = this.state.players.find(p => p.id === playerId);
        if (!player)
            return false;
        ...
        if (x < 50 || x > this.state.mapWidth - 50 || y < 50 || y > this.state.mapHeight - 50)
            return false;
    ...
    handleProduceUnit(playerId, buildingId, unitType) {
        const player = this.state.players.find(p => p.id === playerId);
        const building = this.state.buildings.find(b => b.id === buildingId && b.playerId === playerId);
        if (!player || !building || !building.isConstructed)
            return false;
        // Validate factory / barracks types
        if (building.type === 'nexus' && unitType !== 'harvester')
            return false;
        if (building.type === 'barracks' && unitType !== 'scout' && unitType !== 'assault')
            return false;
        if (building.type === 'factory' && (unitType !== 'heavy_mecha' && unitType !== 'hover_tank' && unitType !== 'dropship'))
            return false;
        const conf = exports.UNIT_CONFIGS[unitType];
        if (!conf)
            return false;
        let metalCost = conf.cost.metal;
    ...
    handleUpgradePlant(playerId, buildingId) {
        const player = this.state.players.find(p => p.id === playerId);
        const building = this.state.buildings.find(b => b.id === buildingId && b.playerId === playerId);
        if (!player || !building || building.type !== 'thermal_plant' || building.isUpgraded)
            return false;
        if (!player.tech.researched.includes('advanced_mining'))
            return false;
        if (player.resources.metal < 100)
            return false;
    ...
    handleResearch(playerId, techId) {
        const player = this.state.players.find(p => p.id === playerId);
        if (!player)
            return false;
        ...
        if (!tech)
            return false;
        if (player.tech.researched.includes(techId) || player.tech.currentlyResearching === techId)
            return false;
        // Check prerequisites
        for (const prereq of tech.prerequisites) {
            if (!player.tech.researched.includes(prereq))
                return false;
        }
        // Check science labs
        ...
        if (!hasPoweredLab)
            return false;
        if (player.resources.science < tech.scienceCost)
            return false;
    ...
    handleActivateUltimate(playerId, targetX, targetY) {
        const player = this.state.players.find(p => p.id === playerId);
        if (!player || player.ultimateCooldown > 0)
            return false;
        if (!player.tech.researched.includes('ultimate_protocol'))
            return false;
        player.ultimateCooldown = player.maxUltimateCooldown;
        if (player.faction === 'nanite') {
            // Reconstitution d'Urgence (10s = 200 ticks)
            player.reconstitutionTicks = 200;
    ...
    solvePowerGrid() {
        ...
        for (const player of this.state.players) {
            ...
            if (player.reconstitutionTicks > 0) {
                player.reconstitutionTicks--;
            }
    ...
    updateBuildings() {
        for (const b of this.state.buildings) {
            ...
            if (!b.isConstructed) {
                ...
                b.constructionProgress += (100 / (exports.BUILDING_CONFIGS[b.type].constructionTicks || 100)) * Math.max(0.2, speed);
                ...
                if (b.constructionProgress >= 100) {
                    ...
                    this.state.log.push(`✅ Construction terminée : ${exports.BUILDING_CONFIGS[b.type].name}.`);
                }
            }
        ...
        for (const player of this.state.players) {
            if (player.tech.currentlyResearching) {
                ...
                if (player.tech.researchProgress >= player.tech.researchTotalTicks) {
                    ...
                    player.tech.researchProgress = 0;
                    const techDef = exports.TECH_TREE[techId];
        ...
        if (unit.order === 'gather' && unit.targetNodeId) {
            ...
            if (nexus) {
                ...
                if (dist <= (nexus.width / 2) + unit.radius + 15) {
                    // Unload cargo
                    if (unit.cargoType === 'metal')
                        player.resources.metal += unit.cargoAmount;
                    else if (unit.cargoType === 'wood')
                        player.resources.wood += unit.cargoAmount;
                    else if (unit.cargoType === 'coal')
                        player.resources.coal += unit.cargoAmount;
                    unit.cargoAmount = 0;
                    // Resume gathering
    ...
    spawnProjectile(shooterId, targetId, startX, startY, targetX, targetY, damage, color, type) {
    ...
    applyDamageToUnit(unit, rawDamage) {
        const player = this.state.players.find(p => p.id === unit.playerId);
        let dmg = rawDamage;
        ...
        if (unit.hp <= 0 && player) {
            const hasReconstitution = player.reconstitutionTicks > 0;
    ...
    applyDamageToBuilding(building, rawDamage) {
        let dmg = rawDamage;
    ...
    updateBotAI() {
        for (const bot of this.state.players.filter(p => p.isBot && p.isAlive)) {
            if (this.state.gameTicks % 40 !== 0)
                continue; // Run every 2s
            ...
            for (const h of harvesters) {
                if (h.order === 'idle') {
                    ...
                    let targetType = 'metal';
                    if (bot.resources.metal > 250 && bot.resources.wood < 80)
                        targetType = 'wood';
                    if (bot.tech.researched.includes('advanced_mining') && bot.resources.coal < 60)
                        targetType = 'coal';
