...
exports.NoteEngine = exports.QUESTION_BANK = void 0;
...
class NoteEngine {
    ...
    addPlayer(socketId, username, color, avatar) {
    ...
    startActivePlayerTurn() {
        ...
        const availableQuestions = exports.QUESTION_BANK.filter(q => !this.state.usedQuestions.includes(q.question));
    ...
    generateBotAnswers() {
        const activePlayer = this.state.players[this.state.activePlayerIndex];
        if (!activePlayer || this.state.secretRating === null)
            return;
        const rating = this.state.secretRating;
        const template = exports.QUESTION_BANK.find(q => q.question === this.state.currentQuestion);
        ...
        nonActiveBots.forEach((bot, index) => {
            ...
            if (template) {
                let tierList;
                if (rating <= 1)
                    tierList = template.botAnswers.tier0;
                else if (rating <= 3)
                    tierList = template.botAnswers.tier1;
                else if (rating <= 5)
                    tierList = template.botAnswers.tier2;
                else if (rating <= 7)
                    tierList = template.botAnswers.tier3;
                else if (rating <= 9)
                    tierList = template.botAnswers.tier4;
                else
                    tierList = template.botAnswers.tier5;
                answerText = tierList[Math.floor(Math.random() * tierList.length)];
            }
            else {
                // Generic fallback for custom questions
                if (rating <= 1)
                    answerText = 'Un désastre total, le pire truc possible ! (0-1/10)';
                else if (rating <= 3)
                    answerText = 'Très médiocre, vraiment pas terrible du tout.';
                else if (rating <= 5)
                    answerText = 'Assez moyen, passable mais rien d\'extraordinaire.';
                else if (rating <= 7)
                    answerText = 'Plutôt bon et agréable, franchement sympa !';
                else if (rating <= 9)
                    answerText = 'Vraiment excellent, très impressionnant et maîtrisé !';
                else
                    answerText = 'La perfection absolue ! Un chef-d\'œuvre légendaire (10/10) !';
            }
    ...
    submitAnswer(playerId, text) {
        ...
        const activePlayer = this.state.players[this.state.activePlayerIndex];
        if (!activePlayer || activePlayer.id === playerId)
            return false; // Active player cannot answer their own question!
    ...
    forceGuessingPhase(requesterId) {
        const activePlayer = this.state.players[this.state.activePlayerIndex];
        if (!activePlayer || activePlayer.id !== requesterId)
            return false;
        if (this.state.phase !== 'ANSWERING' || this.state.answers.length === 0)
            return false;
        this.state.phase = 'GUESSING';
