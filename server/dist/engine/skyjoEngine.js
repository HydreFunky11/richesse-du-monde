...
class SkyjoEngine {
    ...
    drawFromDrawPile(socketId) {
        if (this.state.status !== 'PLAYING' || this.state.drawnCard)
            return false;
    ...
    drawFromDiscardPile(socketId) {
        if (this.state.status !== 'PLAYING' || this.state.drawnCard || this.state.discardPile.length === 0)
            return false;
        const activePlayer = this.state.players[this.state.currentPlayerIndex];
        if (activePlayer.id !== socketId)
            return false;
    ...
    swapWithDiscard(socketId, row, col) {
        if (this.state.status !== 'PLAYING' || this.state.drawnCard || this.state.discardPile.length === 0)
            return false;
        const activePlayer = this.state.players[this.state.currentPlayerIndex];
        if (activePlayer.id !== socketId)
    ...
    discardDrawnCardAndReveal(socketId, row, col) {
        if (this.state.status !== 'PLAYING' || !this.state.drawnCard)
            return false;
        const activePlayer = this.state.players[this.state.currentPlayerIndex];
        if (activePlayer.id !== socketId)
