...
exports.KingOfTokyoEngine = void 0;
const MONSTERS = [
    'Giga-Gros-Chien 🐶',
    'Miaou-Zilla 🐱',
    'Pigeon-Garou 🐦',
...
class KingOfTokyoEngine {
    ...
    toggleKeep(socketId, index) {
        if (this.state.status !== 'PLAYING' || this.state.rollCount === 0 || this.state.rollCount >= 3)
            return false;
        const activePlayer = this.state.players[this.state.currentPlayerIndex];
    ...
    rollDice(socketId) {
        if (this.state.status !== 'PLAYING' || this.state.rollCount >= 3)
            return false;
        const activePlayer = this.state.players[this.state.currentPlayerIndex];
        if (activePlayer.id !== socketId)
            return false;
    ...
    resolveDice(socketId) {
        if (this.state.status !== 'PLAYING' || this.state.rollCount === 0)
            return false;
        const activePlayer = this.state.players[this.state.currentPlayerIndex];
        if (activePlayer.id !== socketId)
            return false;
    ...
    respondYield(socketId, yieldTokyo) {
        if (this.state.status !== 'RESOLVING_ATTACK' || !this.state.pendingYieldRequest)
            return false;
        if (this.state.pendingYieldRequest.tokyoMonsterId !== socketId)
            return false;
    ...
    buyCard(socketId, cardId) {
        ...
        // Refresh store item
        const remainingPool = CARDS_POOL.filter(c => !this.state.store.some(sc => sc.id === c.id) &&
...
exports.KingOfTokyoEngine = KingOfTokyoEngine;
