...
exports.DungeonMayhemEngine = exports.CHARACTERS = void 0;
exports.createDeckForClass = createDeckForClass;
exports.CHARACTERS = {
    barbarian: {
        id: 'barbarian',
...
function createDeckForClass(characterClass) {
    let counter = 1;
...
class DungeonMayhemEngine {
    ...
    addPlayer(id, username, color) {
        ...
        const availableClasses = ['barbarian', 'paladin', 'rogue', 'wizard'];
        ...
        this.state.log.push(`${username} a rejoint l'arène avec ${exports.CHARACTERS[chosenClass].name}.`);
        return true;
    }
    selectCharacter(socketId, characterClass) {
        ...
        this.state.log.push(`${player.username} a choisi ${exports.CHARACTERS[characterClass].name}.`);
        return true;
    }
    ...
    startGame() {
        ...
        this.state.log.push(`⚔️ La bagarre commence ! ${firstPlayer.username} (${exports.CHARACTERS[firstPlayer.characterClass].name}) commence.`);
    ...
    drawCardsToHand(socketId, count) {
        const player = this.state.players.find((p) => p.id === socketId);
        if (!player)
            return;
    ...
    playCard(socketId, cardId, targetPlayerId, targetShieldId) {
    ...
    resolveCard(caster, card, targetPlayerId, targetShieldId) {
        // 1. Healing
        if (card.heal > 0) {
    ...
    applyAttackDamage(attacker, target, amount, preferredShieldId) {
    ...
    nextTurn() {
        ...
        this.state.log.push(`--- C'est au tour de ${nextPlayer.username} (${exports.CHARACTERS[nextPlayer.characterClass].name}) ---`);
    ...
    checkEndGame() {
        ...
        if (alivePlayers.length === 1) {
            ...
            this.state.log.push(`🏆 VICTOIRE ÉPIQUE ! ${alivePlayers[0].username} (${exports.CHARACTERS[alivePlayers[0].characterClass].name}) remporte la bagarre de Dungeon Mayhem !`);
        }
...
exports.DungeonMayhemEngine = DungeonMayhemEngine;
