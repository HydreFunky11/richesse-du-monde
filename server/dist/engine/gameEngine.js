...
class GameEngine {
    ...
    constructor(gameId) {
        this.state = {
            ...
            titles: JSON.parse(JSON.stringify(board_1.INITIAL_TITLES)),
            board: JSON.parse(JSON.stringify(board_1.INITIAL_BOARD)),
            turnNumber: 1,
            lastDiceRoll: null,
    ...
    startGame() {
        ...
        const royaltyPlates = [];
        Object.keys(board_1.RESOURCE_DEFINITIONS).forEach(resType => {
            royaltyPlates.push(resType);
            royaltyPlates.push(resType);
        });
    ...
    rollDice(playerId) {
        ...
        if (!currentPlayer || currentPlayer.id !== playerId)
            return null;
        if (this.state.lastDiceRoll !== null)
            return null;
    ...
    resolveCell(player, cell, diceSum) {
        ...
        if (cell.type === 'RICHESSE') {
            ...
            if (availableTitles.length > 0) {
                this.state.log.push(`Titres disponibles à l'achat pour ${cell.name} : ${availableTitles.map(t => `${board_1.RESOURCE_DEFINITIONS[t.resourceType].name} (${t.purchasePrice.toLocaleString()} F)`).join(', ')}.`);
            }
            else {
    ...
    buyTitle(playerId, titleId) {
        ...
        if (cell.type === 'CHOIX_CONTINENTAL' || cell.type === 'CHOIX_MONDIAL') {
            ...
            if (!ownsAtLeastOne) {
                this.state.log.push(`${currentPlayer.username} ne peut pas acheter ce titre car il ne possède aucune part existante de type ${board_1.RESOURCE_DEFINITIONS[title.resourceType].name}.`);
                return false;
            }
            currentPlayer.cash -= title.purchasePrice;
            title.ownerId = currentPlayer.id;
    ...
    // --- LOGIQUE ENCHÈRES ---
    startAuction(playerId, titleIdsToAuction) {
        const currentPlayer = this.getCurrentPlayer();
        if (!currentPlayer || currentPlayer.id !== playerId)
            return false;
        if (this.state.status !== 'PLAYING')
            return false;
        const cell = this.state.board[currentPlayer.position];
        if (cell.type !== 'ENCHERES' || currentPlayer.lapsCompleted < 1)
            return false;
        ...
        for (const tId of titleIdsToAuction) {
            ...
            if (title && title.ownerId === currentPlayer.id) {
                if (!resourceTypesSeen.has(title.resourceType)) {
                    ...
                    const trustTitles = Object.values(this.state.titles).filter(t => t.resourceType === title.resourceType && t.ownerId === currentPlayer.id);
        ...
        this.state.log.push(`[ENCHÈRES] L'enchère commence pour : ${finalTitlesToAuction.map(id => `${this.state.titles[idxToKey(id)].country} (${board_1.RESOURCE_DEFINITIONS[this.state.titles[id].resourceType].name})`).join(', ')}.`);
        this.state.log.push(`Prix de départ : ${startingPrice.toLocaleString()} F.`);
        return true;
    }
    placeBid(playerId, bidAmount) {
        if (this.state.status !== 'AUCTION' || !this.state.auction)
            return false;
        const bidder = this.state.players.find(p => p.id === playerId);
        if (!bidder || bidder.isBankrupt || bidder.id === this.state.auction.sellerId)
            return false;
        ...
        if (bidAmount < minBid || bidder.cash < bidAmount)
            return false;
    ...
    passBid(playerId) {
        if (this.state.status !== 'AUCTION' || !this.state.auction)
            return false;
        const bidder = this.state.players.find(p => p.id === playerId);
        if (!bidder || bidder.id === this.state.auction.sellerId)
            return false;
