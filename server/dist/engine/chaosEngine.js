...
class ChaosEngine {
    ...
    playAction(socketId, actionType, params = {}) {
        const activePlayer = this.state.players[this.state.currentPlayerIndex];
    ...
    modifyCell(socketId, cellIndex, newType) {
    ...
    checkElimination(player, reason) {
        if (player.health <= 0 || player.debt >= 2000) {
            ...
            this.state.log.push(`💀💀💀 ${player.username.toUpperCase()} A ÉTÉ ÉLIMINÉ (${reason}) ! LA MANCHE S'ARRÊTE ! 💀💀💀`);
            this.state.log.push(`📜 ${player.username} devient le Législateur du Chaos et prépare une nouvelle règle pour la prochaine manche...`);
        }
    }
    // ─── DRAFTING NEW RULE (CALLED FROM CLIENT / AI) ───────────────────────────
    async submitNewRule(socketId, ruleText) {
        ...
        try {
            ...
            const parsedRule = await (0, chaosAi_1.interpretChaosRule)(ruleText, author, this.state.roundNumber);
    ...
    evaluateRules(trigger, ctx) {
        for (const rule of this.state.activeRules) {
            if (rule.trigger !== trigger)
                continue;
            // Check condition
            if (!this.checkRuleCondition(rule.condition, ctx))
                continue;
    ...
    checkRuleCondition(cond, ctx) {
        if (!cond || cond.type === 'ALWAYS')
            return true;
    ...
    resolveEffectTargets(targetType, ctx) {
