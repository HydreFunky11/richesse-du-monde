...
const DECK_TEMPLATE: ClashCardId[] = [
  'knight',
  'archers',
  'skeletons',
  'giant',
...
export class ClashEngine {
  ...
  private state: ClashGameState;
  private loopInterval: NodeJS.Timeout | null = null;
  private onStateUpdateCallback: ((state: ClashGameState) => void) | null = null;
  private lastTickTime: number = 0;
  private botLastPlayTime: number = 0;
  ...
  private createInitialState(): ClashGameState {
    return {
      status: 'LOBBY',
  ...
  public getState(): ClashGameState {
    return this.state;
  }

  public getPlayers(): ClashPlayer[] {
  ...
  public playCard(playerId: string, cardId: ClashCardId, x: number, y: number): boolean {
    ...
    if (player.elixir < cardDef.cost) return false;
  ...
  private spawnCardEntities(team: ClashTeam, cardId: ClashCardId, x: number, y: number) {
    const now = Date.now();
  ...
  public startLoop(callback: (state: ClashGameState) => void) {
    this.stopLoop();
  ...
  private updateTimerAndElixir(dt: number) {
    this.state.timer -= dt;
  ...
  private detonateProjectile(p: ClashProjectile) {
    const enemyTeam: ClashTeam = p.team === 'blue' ? 'red' : 'blue';
      ...
      this.state.towers.forEach(t => {
        if (t.team === enemyTeam && t.hp > 0) {
          ...
          if (d <= p.aoeRadius! + 4) {
            ...
            if (t.type === 'king' && !t.isActive) t.isActive = true;
          }
        }
      });
  ...
  private findTargetForUnit(unit: ClashUnit): { x: number; y: number; id: string; isTower: boolean } | null {
    const enemyTeam: ClashTeam = unit.team === 'blue' ? 'red' : 'blue';

    // 1. If targetOnlyBuildings -> only look for enemy towers and enemy buildings
    if (unit.targetOnlyBuildings) {
      let closest: { x: number; y: number; id: string; isTower: boolean } | null = null;
    ...
    let closestUnit: ClashUnit | null = null;
    let minUnitDist = 22;

    for (const enemy of this.state.units) {
      if (enemy.team === enemyTeam && enemy.hp > 0) {
        if (enemy.isFlying && !unit.targetAir) continue; // cannot target flying
    ...
    if (closestTower) {
      const ct = closestTower as ClashTower;
  ...
  private moveUnitTowards(unit: ClashUnit, targetX: number, targetY: number, dt: number) {
    let destX = targetX;
    let destY = targetY;
  ...
  private executeUnitAttack(unit: ClashUnit, target: { x: number; y: number; id: string; isTower: boolean }) {
    if (unit.aoeRadius) {
  ...
  private processUnitAttack(unit: ClashUnit, now: number) {
    if (now - unit.lastAttackTime < unit.attackSpeed * 1000) return;
    const target = this.findTargetForUnit(unit);
    if (!target) return;
    const dist = Math.sqrt((target.x - unit.x) ** 2 + (target.y - unit.y) ** 2);
    if (dist <= unit.range) {
  ...
  private updateTowers(now: number) {
    this.state.towers.forEach(tower => {
      ...
      if (tower.type === 'king' && !tower.isActive) return;

      if (now - tower.lastAttackTime >= tower.attackSpeed * 1000) {
        const enemyTeam: ClashTeam = tower.team === 'blue' ? 'red' : 'blue';
        let closest: ClashUnit | null = null;
        ...
        if (closest) {
          ...
          this.state.projectiles.push({
            ...
            currentX: tower.x,
            currentY: tower.y,
            targetX: (closest as ClashUnit).x,
            targetY: (closest as ClashUnit).y,
            speed: 55,
            damage: tower.damage,
            type: tower.type === 'king' ? 'cannon' : 'arrow',
            team: tower.team,
            targetUnitId: (closest as ClashUnit).id
          });
        }
      }
    });
  ...
  private finishGame(winnerTeam: ClashTeam | 'DRAW', logMessage: string) {
    this.stopLoop();
    this.state.status = 'FINISHED';
