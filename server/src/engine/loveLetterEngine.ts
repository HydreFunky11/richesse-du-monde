import { LoveLetterGameState, LoveLetterPlayer, LoveLetterCard, LoveLetterCardType } from '../types/loveletter';
import { v4 as uuidv4 } from 'uuid';

const CARD_TEMPLATES: { type: LoveLetterCardType; value: number; name: string; description: string; count: number }[] = [
...
export class LoveLetterEngine {
  ...
  private state: LoveLetterGameState;
  ...
  public getPlayers(): LoveLetterPlayer[] {
    return this.state.players;
  }

  public getState(): LoveLetterGameState {
    return this.state;
  }
  ...
  private startNewRound() {
    ...
    if (this.state.players.length === 2) {
      for (let i = 0; i < 3; i++) {
        const c = cards.pop();
        if (c) this.state.burnCards.push(c);
      }
  ...
  private drawCardForCurrentPlayer() {
  ...
  public playCard(socketId: string, cardId: string, targetPlayerId?: string, guessedCardType?: LoveLetterCardType): boolean {
  ...
  private resolveCardEffect(type: LoveLetterCardType, actor: LoveLetterPlayer, targetId: string, guessed?: LoveLetterCardType) {
  ...
  private eliminatePlayer(p: LoveLetterPlayer, reason: string) {
    p.isEliminated = true;
  ...
  private resolveRoundEnd() {
    ...
    let bestVal = -1;
    let roundWinners: LoveLetterPlayer[] = [];
  ...
  private awardRoundWinner(winner: LoveLetterPlayer) {
    winner.tokens += 1;
  ...
  public nextRound(): boolean {
    if (this.state.status !== 'ROUND_END') return false;
    this.state.status = 'PLAYING';
    this.startNewRound();
    return true;
  }
