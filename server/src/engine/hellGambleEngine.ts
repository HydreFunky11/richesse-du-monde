import {
  ...
  LiveDrop,
  SkinRarity,
} from '../types/hellgamble';
import {
  CASES_DATABASE,
  SKINS_DATABASE,
  getSkinDefinition,
  generateSkinInstance,
  rollCaseDrop
} from '../data/skinsData';

export class HellGambleEngine {
  private roomCode: string;
  private players: Map<string, HellPlayer> = new Map();
  private liveFeed: LiveDrop[] = [];
  private activeBattles: CaseBattle[] = [];
  private recentBattles: CaseBattle[] = [];
  private onUpdate: (state: HellGambleState) => void;

  constructor(roomCode: string, onUpdate: (state: HellGambleState) => void) {
  ...
  public addPlayer(id: string, username: string, avatar?: string): HellPlayer {
  ...
  public getPlayer(id: string): HellPlayer | undefined {
    return this.players.get(id);
  }

  public getPlayers(): HellPlayer[] {
    return Array.from(this.players.values());
  }

  private recalculateNetWorth(player: HellPlayer) {
  ...
  public getState(): HellGambleState {
    return {
  ...
  public sellItem(playerId: string, itemId: string): { success: boolean; error?: string; soldValue?: number } {
    const player = this.players.get(playerId);
    if (!player) return { success: false, error: 'Joueur introuvable.' };

    const itemIdx = player.inventory.findIndex(i => i.id === itemId);
    if (itemIdx === -1) return { success: false, error: 'Item introuvable dans l\'inventaire.' };
  ...
  public sellAll(playerId: string, maxPrice?: number): { success: boolean; count: number; totalGained: number } {
    const player = this.players.get(playerId);
    if (!player) return { success: false, count: 0, totalGained: 0 };
  ...
  public claimBankruptBonus(playerId: string): { success: boolean; error?: string; bonusAmount?: number } {
    const player = this.players.get(playerId);
    if (!player) return { success: false, error: 'Joueur introuvable.' };
  ...
  public upgrade(
    ...
    const targetDef = getSkinDefinition(targetSkinId);
    if (!targetDef) return { success: false, error: 'Skin cible invalide.' };

    // Calculer la mise
    let totalWager = 0;
    const itemsToBurn: HellItem[] = [];

    for (const itemId of wagerItemIds) {
      const item = player.inventory.find(i => i.id === itemId);
      if (!item) return { success: false, error: 'Item sélectionné introuvable.' };
      itemsToBurn.push(item);
      totalWager += item.value;
    }

    if (cashWager > 0) {
      if (player.cash < cashWager) return { success: false, error: 'Fonds insuffisants pour la mise cash.' };
    ...
    let targetInstance: HellItem | undefined;

    if (won) {
      const generated = generateSkinInstance(targetDef.id, 'Upgrader');
      targetInstance = {
  ...
  public tradeUp(playerId: string, itemIds: string[]): { success: boolean; error?: string; itemWon?: HellItem } {
    const player = this.players.get(playerId);
    if (!player) return { success: false, error: 'Joueur introuvable.' };
    ...
    const items: HellItem[] = [];
    for (const id of itemIds) {
      const item = player.inventory.find(i => i.id === id);
      if (!item) return { success: false, error: 'Un ou plusieurs skins sélectionnés sont introuvables.' };
      items.push(item);
    }
    ...
    const candidateSkins = SKINS_DATABASE.filter(s => s.rarity === targetRarity);
    ...
    const generated = generateSkinInstance(chosenDef.id, "Contrat d'échange");

    const wonItem: HellItem = {
  ...
  public createBattle(
    ...
    caseIds: string[],
    maxPlayers: 2 | 3 | 4
  ): { success: boolean; error?: string; battle?: CaseBattle } {
    const player = this.players.get(playerId);
    if (!player) return { success: false, error: 'Joueur introuvable.' };
    ...
    for (const cId of caseIds) {
      ...
      if (!cDef) return { success: false, error: `Caisse invalide: ${cId}` };
  ...
  public joinBattle(playerId: string, battleId: string): { success: boolean; error?: string } {
    const player = this.players.get(playerId);
    if (!player) return { success: false, error: 'Joueur introuvable.' };
  ...
  public leaveBattle(playerId: string, battleId: string): { success: boolean; error?: string } {
    const battle = this.activeBattles.find(b => b.id === battleId);
    if (!battle) return { success: false, error: 'Battle introuvable.' };
    ...
    if (pIndex === -1) return { success: false, error: 'Vous ne participez pas à cette battle.' };
  ...
  public startBattle(playerId: string, battleId: string): {
    success: boolean;
    error?: string;
    battleResults?: {
      rounds: {
      ...
      for (const participant of battle.participants) {
        ...
        for (let i = 0; i < 45; i++) {
          if (i === winningIndex) {
            ...
            const rand = rollCaseDrop(caseDef);
            reelItems.push({
              skinId: rand.skin.id,
              name: rand.skin.name,
    ...
    for (const p of battle.participants) {
      ...
      if (pl) this.recalculateNetWorth(pl);
    }
  ...
  private addLiveFeed(player: HellPlayer, item: HellItem, source: 'CASE' | 'BATTLE' | 'UPGRADE' | 'TRADEUP') {
