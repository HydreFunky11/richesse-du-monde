import { ChaosRule, ChaosStatDef } from '../types/chaos';
...
export function buildChaosDynamicSystemPrompt(
  ...
  const rulesSummary = activeRules.length === 0
    ...
    : activeRules.map((r, i) =>
        `#${i + 1} [Manche ${r.roundIntroduced}] "${r.title}" (par ${r.authorName}) : ${r.description} (Déclencheur: ${r.trigger})`
      ).join('\n');
