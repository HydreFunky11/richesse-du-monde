import { ChaosRule, ChaosRuleEffect, ChaosRuleCondition, ChaosBoardMod, ChaosAiLog, ChaosCellType } from '../types/chaos';
...
export function getCellDefaultMeta(type: ChaosCellType): { name: string; icon: string; description: string } {
  switch (type) {
    case 'LAVA':
...
export function generateFallbackRule(
  ...
  if (isCaseRequest && (isAdd || isModify)) {
    trigger = 'ON_ROUND_START';
    let targetType: ChaosCellType = 'CHAOS';
