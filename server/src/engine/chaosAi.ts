import { ChaosRule, ChaosRuleEffect, ChaosBoardMutation, ChaosAiLog, ChaosGameState } from '../types/chaos';
import { buildChaosDynamicSystemPrompt } from './chaosPromptContext';
...
export async function interpretChaosRule(
  ...
  authorName: string,
  roundNumber: number,
  currentGameState?: ChaosGameState,
  onLog?: (log: ChaosAiLog) => void
): Promise<ChaosRule> {
...
export function generateFallbackRule(
  ...
  let trigger: ChaosRule['trigger'] = 'ON_MOVE';
  const effects: ChaosRuleEffect[] = [];
  const boardMutations: ChaosBoardMutation[] = [];
