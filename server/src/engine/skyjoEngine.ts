...
export class SkyjoEngine {
  ...
  public getPlayers(): SkyjoPlayer[] {
    return this.state.players;
  }

  public getState(): SkyjoGameState {
    return this.state;
  }
  ...
  private generateDeck(): SkyjoCard[] {
  ...
  public revealCardInitial(socketId: string, row: number, col: number): boolean {
    if (this.state.status !== 'REVEAL_TWO') return false;
    ...
    let faceUpCount = 0;
    player.grid.forEach(r => r.forEach(c => { if (c.faceUp) faceUpCount++; }));
    ...
    const everyoneReady = this.state.players.every(p => {
      let count = 0;
      p.grid.forEach(r => r.forEach(c => { if (c.faceUp) count++; }));
      return count >= 2;
    });
    ...
    if (everyoneReady) {
      ...
      this.state.players.forEach((p, idx) => {
        let sum = 0;
        p.grid.forEach(r => r.forEach(c => { if (c.faceUp) sum += c.value; }));
        if (sum > highestSum) {
          highestSum = sum;
          startIdx = idx;
        }
  ...
  private checkColumnAlignment(player: SkyjoPlayer) {
  ...
  private endTurnTasks(player: SkyjoPlayer) {
    this.checkColumnAlignment(player);

    // Check if player has finished (all remaining cards are faceUp)
    let allFaceUp = true;
    player.grid.forEach(r => r.forEach(c => { if (!c.faceUp) allFaceUp = false; }));
  ...
  private endRound() {
    ...
    if (maxScore >= 100) {
      ...
      let minScore = 9999;
      let winnerPlayer: SkyjoPlayer | null = null;
