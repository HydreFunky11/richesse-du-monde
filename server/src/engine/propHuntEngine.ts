...
const MAP_PROPS: Record<PropHuntMapId, string[]> = {
  superette: [
    'soda_can',
    'cereal_box',
    'shopping_cart',
...
export class PropHuntEngine {
  private roomCode: string;
  private phase: 'LOBBY' | 'HIDING' | 'HUNTING' | 'FINISHED' = 'LOBBY';
  private players: PropHuntPlayer[] = [];
  private selectedMap: PropHuntMapId = 'superette';
  private mapVotes: Record<string, PropHuntMapId> = {};
  private hidingTimeRemaining: number = 25; // 25s for props to hide
  private roundTimeRemaining: number = 300; // 5 min minimum
  private roundDuration: number = 300;
  private hunterId: string | null = null;
  private winner: 'HUNTER' | 'HIDERS' | null = null;
  private activeTaunts: PropHuntTaunt[] = [];
  private logs: string[] = [];
  private lastAutoTauntCheck: number = 0;
  private intervalTimer: NodeJS.Timeout | null = null;
  private onStateChange?: (state: PropHuntGameState) => void;

  constructor(roomCode: string, onStateChange?: (state: PropHuntGameState) => void) {
    this.roomCode = roomCode;
    this.onStateChange = onStateChange;
  }

  public getPlayers(): PropHuntPlayer[] {
    return this.players;
  }
  ...
  public voteMap(playerId: string, mapId: PropHuntMapId) {
    if (this.phase !== 'LOBBY') return;
    this.mapVotes[playerId] = mapId;
    this.broadcast();
  }

  public startGame(initiatorId: string): { success: boolean; error?: string } {
    if (this.phase !== 'LOBBY' && this.phase !== 'FINISHED') {
    ...
    // Tally votes
    const voteCounts: Record<PropHuntMapId, number> = {
      superette: 0,
      warehouse: 0,
      office: 0,
      lab: 0
    ...
    Object.values(this.mapVotes).forEach(m => {
      if (voteCounts[m] !== undefined) voteCounts[m]++;
    });

    let bestMap: PropHuntMapId = 'superette';
    let maxVotes = -1;
    (Object.keys(voteCounts) as PropHuntMapId[]).forEach(m => {
  ...
  private triggerAutomaticTaunts() {
  ...
  public changeProp(playerId: string): { success: boolean; newProp?: string; cooldownRemaining?: number } {
    const player = this.players.find(p => p.id === playerId);
    if (!player || player.role !== 'HIDER' || player.health <= 0) {
      return { success: false };
    }
  ...
  public toggleFreeze(playerId: string): boolean {
    const player = this.players.find(p => p.id === playerId);
    if (!player || player.role !== 'HIDER' || player.health <= 0) return false;
  ...
  public triggerManualTaunt(playerId: string): boolean {
    const player = this.players.find(p => p.id === playerId);
    if (!player || player.role !== 'HIDER' || player.health <= 0) return false;

    const now = Date.now();
    if (now - player.lastTauntTime < 5000) return false; // 5s min cooldown between manual taunts
  ...
  ): { hit: boolean; killed: boolean; hunterDamageTaken: number } {
    if (this.phase !== 'HUNTING') {
  ...
  public getState(): PropHuntGameState {
    return {
